...
public class BookFactory implements BookFacade {
    ...
    public BookFactory (Collection<Book> c) {
       books = c;
    }
    ...
    public BookFactory filter(String a) {
        ArrayList<Book> bnms = new ArrayList<Book>();
        for(Book b : books){
            if(b.getAuthor().charAt(0)==a.charAt(0)){
                bnms.add(b);
            }
        }
        BookFactory bf =new BookFactory(bnms);
